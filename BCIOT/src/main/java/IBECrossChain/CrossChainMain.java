...
public class CrossChainMain {
    public static void main(String[] args) throws Exception {
        ...
        //获取getService工具类对象
        crosschain.GetService getService = new GetService();
        ////获取链1服务
        ...
        //socket通信 ip地址
        String ip = getService.getSplit(data, 4);
        //进行socket通信
        Socket socket_ibe = new Socket(ip, 8888);
        OutputStream os = socket_ibe.getOutputStream();
        InputStream is = socket_ibe.getInputStream();
        os.write(message_digest.getBytes());
        byte[] bytes = new byte[1024];
        int len = is.read(bytes);
        //返回的私钥 公钥 密文
        String return_result = new String(bytes, 0, len);
        System.out.println("return_result: " + return_result);
        socket_ibe.close();
        //私钥
        String PrivateKey = getService.getSplit(return_result, 0);
        //公钥
        String PublicKey = getService.getSplit(return_result, 1);
        //密文
        String encrypt_text = getService.getSplit(return_result, 2) + "," + getService.getSplit(return_result, 3);
        System.out.println("PrivateKey: " + PrivateKey);
        System.out.println("PublicKey: " + PublicKey);
        System.out.println("encrypt_text: " + encrypt_text);

        //与链2通信 将原文的摘要信息 公钥 密文 ibe服务端ip 发送给链2
        Socket socket_chain2 = new Socket(ChainIP, 8887);
        OutputStream os1 = socket_chain2.getOutputStream();
        os1.write((PublicKey + "," + encrypt_text + "," + ip + "," + message_digest).getBytes());
    }
}
